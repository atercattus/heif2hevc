...
int main(int argc, char *argv[]) {
    ...
    if (not properties.fileFeature.hasFeature(ImageFileReaderInterface::FileFeature::HasRootLevelMetaBox)) {
        fprintf(stderr, "no meta_box\n");
        return 2;
    } else if (not fullImage and not tiledImage) {
    ...
    if (fullImage) { // master
        ...
        for (auto itemId : itemIds) {
            ...
            snprintf(tail, 1000, ".%03d.tile", itemId);
